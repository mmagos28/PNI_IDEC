...
if nargin == 0
    ...
    if exist(localCoordsPath, 'file')
        coords = load(localCoordsPath);
...
if ~exist('h4','var') || isempty(h4) || ~ishandle(h4)
...
% XProjectorMax = 912/1140; %912/1140; %608/684;%/608; %SR
% XProjectorMin = -912/1140;% -912/1140; %-608/684;%/608; %SR
XProjectorMax = 1140/912;
XProjectorMin = -1140/912;
YProjectorMax = 1; %SR
YProjectorMin = -1;
...
%         plot(HorizPx,VerticPx,'ro','LineWidth',5)
        plot(HorizPx - x2, VerticPx - y2,'go','LineWidth',5)   
    elseif elevat == -45* pi/180
        plot(HorizPx - x2, VerticPx - y2,'yx','LineWidth',5)
    else
...
%%% Edited: SR
plot(XProjectorMin - x2, YProjectorMin - y2,'w*', 'MarkerSize', 14);
plot(XProjectorMin - x2, YProjectorMax - y2,'w*', 'MarkerSize', 14);
plot(XProjectorMax - x2, YProjectorMin - y2,'w*', 'MarkerSize', 14);
plot(XProjectorMax - x2, YProjectorMax - y2,'w*', 'MarkerSize', 14);
...
usedWindowAPI = false;
if exist('WindowAPI', 'file') == 3 || exist('WindowAPI', 'file') == 2
    try
        WindowAPI(h4, 'Position', FigPos,1);
...
if ~usedWindowAPI
    ...
    set(h4, 'Units', 'pixels', 'Position', FigPos);
    movegui(h4, 'onscreen');
    set(h4, 'OuterPosition', monitorPosition);
end
