function [h4]=generateDomeFromCoords(Rs,xsm,ysm,zsm, r,xOm,yOm,zOm, xP1o,yP1o,zP1o, x0,y0,x1,y1,x2,y2,h4)
...
if nargin == 0
    ...
    elseif exist('coords.mat', 'file')
        ...
        error('generateDome:MissingInputs', ...
            ['generateDome requires calibration coordinates. ', ...
             'Run ConfigureProjector and click Save Coords first, or call ', ...
             'generateDome with all geometry arguments.']);
    end
    ...
    yOm = coords.mirrorY_;
    zOm = coords.mirrorZ_;
    xP1o = coords.projectorX_ - xOm;
    yP1o = coords.projectorY_ - yOm;
    zP1o = coords.projectorZ_ - zOm;
    x0 = coords.x0_;
    y0 = coords.y0_;
    x1 = coords.x1_;
    y1 = coords.y1_;
...
outputPath = fullfile(fileparts(mfilename('fullpath')), 'calibratedBallImage.data');
f = fopen(outputPath,'w');
if f < 0
    error('generateDome:OutputOpenFailed', 'Could not open calibration output file: %s', outputPath);
end
fprintf(f,'2\n');
% fprintf(f,'%d %d\n',numel1, numel2);
% fprintf(f,'%d %d\n',numel2, numel1);
