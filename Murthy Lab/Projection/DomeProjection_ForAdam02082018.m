...
canRescaleX = exist('xm0','var') && exist('xm2','var') && (x2 ~= x0) && (xm2 ~= xm0);
canRescaleY = exist('ym0','var') && exist('ym1','var') && (y1 ~= y0) && (ym1 ~= ym0);

if canRescaleX
    NewXscaleExtent=(Innerposition(3)-Innerposition(1))*(xm2-xm0)/(x2-x0);
    NewXscalMin=(-x0+xm0*(x2-x0)/(xm2-xm0))/((x2-x0)/(xm2-xm0));
    newXLim = sort([NewXscalMin NewXscalMin+NewXscaleExtent]);
    if all(isfinite(newXLim)) && newXLim(1) < newXLim(2)
        xlim(newXLim);
    else
        warning('DomeProjection:InvalidXLimits', 'Computed X limits are invalid. Keeping existing xlim.');
    end
else
    warning('DomeProjection:MissingXCalibration', ...
        'X calibration points are not valid for rescaling. Keeping existing xlim.');
end

if canRescaleY
    NewYscaleExtent=(Innerposition(4)-Innerposition(2))*(ym1-ym0)/(y1-y0);
    NewYscalMin=(-y0+ym0*(y1-y0)/(ym1-ym0))/((y1-y0)/(ym1-ym0));
    newYLim = sort([NewYscalMin NewYscalMin+NewYscaleExtent]);
    if all(isfinite(newYLim)) && newYLim(1) < newYLim(2)
        ylim(newYLim);
    else
        warning('DomeProjection:InvalidYLimits', 'Computed Y limits are invalid. Keeping existing ylim.');
    end
else
    warning('DomeProjection:MissingYCalibration', ...
        'Y calibration points are not valid for rescaling. Keeping existing ylim.');
end
