...
try
   ...
   warning('demo_WindowAPI:UnavailableWindowAPI', ...
      ['WindowAPI is on the MATLAB path, but it could not initialize: %s\n', ...
       'Showing a simple MATLAB-only window positioning demo instead.'], ME.message);
   showMatlabOnlyWindowAPIFallback();
   return;
end

if isempty(whichWindowAPI)
   warning('demo_WindowAPI:MissingMex', ...
      ['WindowAPI.mex is not found in the MATLAB path. ', ...
       'Showing a simple MATLAB-only window positioning demo instead.']);
   showMatlabOnlyWindowAPIFallback();
   return;
end
fprintf('Using: %s\n\n', whichWindowAPI);
...
% return;
function showMatlabOnlyWindowAPIFallback()
FigH = figure('Color', ones(1, 3), 'Renderer', 'Painters', ...
   'Name', 'WindowAPI unavailable');
axes('Visible', 'off', 'Units', 'normalized', 'Position', [0, 0, 1, 1]);
text(0.5, 0.5, {'WindowAPI is not available.', ...
   'Install/compile WindowAPI.mex to run the full demo.'}, ...
   'Units', 'normalized', ...
   'FontSize', 16, ...
   'HorizontalAlignment', 'center', ...
